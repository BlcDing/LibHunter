...
def extractHTTPMetadata(captureLog, db_connection):
    ...
    for tcpFlow in trace.trace:
        ...
        while len(tcpFlowIndex) > 0:
            ...
            httpMessage = {'srcAddr': '', 'srcPort': None, 'dstAddr': '', 'dstPort': None,
                           ...
                           'requestTime': '', 'responseTime': '', 'four_tuple_hash': '',}
            # 偏移量
            index = 0
            requestBodyLength = 0
            ...
            while True:
                ...
                if pcapkit.HTTP in frame:
                    ...
                    if http.info.receipt == 'request' and len(httpMessage['requestHeaders']) == 0:
                        ### requestTime
                        requestTime = str(frame.info.time.strftime('%d/%m/%Y %I:%M:%S %p'))
                        if requestTime.endswith("AM"):
                            requestTime = requestTime[:-2] + "上午"
                        else:
                            requestTime = requestTime[:-2] + "下午"
                        httpMessage['requestTime'] = requestTime
                        ...
                        ### responseTime
                        responseTime = str(frame.info.time.strftime('%d/%m/%Y %I:%M:%S %p'))
                        if responseTime.endswith("AM"):
                            responseTime = responseTime[:-2] + "上午"
                        else:
                            responseTime = responseTime[:-2] + "下午"
                        httpMessage['responseTime'] = responseTime
    ...
    for httpMessage in httpMessageList:
        ...
        four_tuple_hash = "-".join([
            str(httpMessage['srcAddr']),
            str(httpMessage['dstAddr']),
            str(httpMessage['srcPort']),
            str(httpMessage['dstPort']),
        ])
        httpMessage["four_tuple_hash"] = "+".join([
            str(httpMessage['packageName']),
            four_tuple_hash
        ])


        httpMessageListFinal.append([
            httpMessage['packageName'],
            httpMessage['srcAddr'],
            httpMessage['srcPort'],
            httpMessage['dstAddr'],
            httpMessage['dstPort'],
            httpMessage['host'],
            httpMessage['URL'],
            httpMessage['requestTime'],
            httpMessage['responseTime'],
